...
while os.path.exists(r'fort.' + str(i+1) + '.csv') and os.path.exists(r'fort.' + str(i+1) + 'B.csv'):
    ...
    for ypoint in range(ypoints):
        for xpoint in range(xpoints):
            ...
            isInLine = False
            while walk < 2:
                linex = float(bismuth[0]) - walk * (float(bismuth[0]) - 6.912078571)
                liney = float(bismuth[1]) - walk * (float(bismuth[1]) - 4.46801)
                linez = float(bismuth[2]) - walk * (float(bismuth[2]) - 4.59348)
